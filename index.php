...
<head>
    ...
    <style>
        ...
        .order-button:hover {
            background: linear-gradient(to right, #332E2D, #0A4854);
            text-decoration: none;
            margin-top: 2px;


            color: #fff;/* Darker shade on hover, you can customize */
        }
    </style>
</head>
...
<?php
  ...
  <header>
    ...
    if (isset($_SESSION['username'])) {
        ...
        echo "<div class='user-info'>Guest</div>"; // Display something if username session is not set
    }
    ?>
</header>
    ...
    <div class="container">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                if (!empty($row['url'])) {
                    echo "<a href='" . $row['url'] . "' target='_blank' class='order-button'>Go to Page</a>";
                } else {
